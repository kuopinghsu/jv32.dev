...
std::string RiscvDisassembler::disassemble(uint32_t instr, uint32_t pc) {
    ...
    switch (opcode) {
        ...
        case 0x0B: { // OPCODE_CUSTOM0: Zcmt cm.jt / cm.jalt
            // Encoding: {4'h0, index[7:0], 5'h0, 3'b001, rd[4:0], 7'b0001011}
...
std::string RiscvDisassembler::decode_r_type(uint32_t instr, uint32_t opcode, uint32_t funct3, uint32_t funct7) {
    ...
    if (opcode == 0x33) { // OP
        if (funct7 == 0x00) {
            ...
            if (funct3 == 0x4 && rs2 == 0) mnemonic = "zext.h"; // ZEXT.H (rs2=x0)
        ...
        switch (funct3) {
            ...
            case 0x1:
                if (funct7 == 0x00) {
                    ...
                    // Zbb: single-operand count/extend instructions (CLZ/CTZ/CPOP/SEXT.B/SEXT.H)
                    ...
                    mnemonic = "bclri"; imm = shamt;
                } else if (funct7 == 0x34) { // Zbs: BINVI
                    mnemonic = "binvi"; imm = shamt;
                } else if (funct7 == 0x14) { // Zbs: BSETI
                    mnemonic = "bseti"; imm = shamt;
                }
                break;
    ...
    switch (funct3) {
        ...
        case 0x4: { // Arithmetic
            ...
            if (funct2 == 0x0) { // C.SRLI
                ...
                } else if (funct1 == 1) { // Zcb: c.mul / unary ops
                    uint32_t f2_low = (instr >> 5) & 0x3;
                    if (f2_low == 0x2) { // c.mul
                        oss << "c.mul " << c_reg_name(rd_rs1_p) << "," << c_reg_name(rs2_p);
...
std::string RiscvDisassembler::decode_c_quadrant2(uint16_t instr) {
    ...
    switch (funct3) {
        ...
        case 0x5: { // Zcmt cm.jt/cm.jalt  |  Zcmp push/pop/mv*
            uint32_t ci_12_10 = (instr >> 10) & 0x7;
            if (ci_12_10 == 0x0) { // Zcmt: ci[12:10]=000
            ...
            if (ci_12_10 == 0x3) { // Zcmp: cm.mvsa01 / cm.mva01s
                uint32_t ci_6_5 = (instr >> 5) & 0x3;
                uint32_t r1s = (instr >> 7) & 0x7;
                uint32_t r2s = (instr >> 2) & 0x7;
                static const char* sn[8] = {"s0","s1","s2","s3","s4","s5","s6","s7"};
                if (ci_6_5 == 0x1)
                    oss << "cm.mvsa01 " << sn[r1s] << "," << sn[r2s];
                else if (ci_6_5 == 0x3)
                    oss << "cm.mva01s " << sn[r1s] << "," << sn[r2s];
                else return "unknown";
                return oss.str();
            }
            // Zcmp: push/pop family: ci[12]=1, ci[11]=1, ci[8]=0
