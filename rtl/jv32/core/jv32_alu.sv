...
`endif
    ...
    generate
        if (FAST_MUL == 1) begin : gen_fast_mul
            ...
            logic _unused_mul; assign _unused_mul = &{1'b0,result_mulu[31:0],result_mulsu[31:0]};
                ...
                if (!rst_n) begin
                    ...
                    if (is_mul_op && !mul_active && !mul_valid && clz_b_mul!=6'd32 && !operand_stall) begin
                        ...
                        if (mul_count+1 >= mul_total) begin
                            ...
                            mul_valid  <= 1'b1; mul_active <= 1'b0;
                        end else begin
    ...
    generate
        if (FAST_DIV == 1) begin : gen_fast_div
            ...
            assign signed_ovf  = (operand_a==32'h80000000) && (operand_b==32'hffffffff);
            ...
            always_ff @(posedge clk or negedge rst_n) begin
                if (!rst_n) begin
                    div_count<=6'd0; div_total<=6'd0; div_q<=32'd0; div_r<=32'd0; div_abs_b<=32'd0;
                    div_valid<=1'b0; div_active<=1'b0; div_neg_q<=1'b0; div_neg_r<=1'b0;
                    ...
                    if (is_div_op && !div_active && !div_valid && !operand_stall) begin
                        if (!div_by_zero && !signed_ovf) begin
                            ...
                            div_q<=abs_a<<clz_a; div_r<=32'd0; div_abs_b<=abs_b;
                            div_neg_q<=is_signed_div&&(operand_a[31]^operand_b[31]);
                            div_neg_r<=is_signed_div&&operand_a[31];
                            div_by_zero_lat<=div_by_zero; div_signed_ovf_lat<=signed_ovf;
                            div_operand_a_lat<=operand_a;
                            if (clz_a==6'd32) begin
                                div_result_q<=32'd0; div_result_r<=32'd0; div_valid<=1'b1;
                            end else div_active<=1'b1;
                        end
                    end else if (div_active) begin
                        div_q<=next_q; div_r<=next_r;
                        if (div_count+1>=div_total) begin
                            div_result_q<=div_neg_q?(~next_q+1):next_q;
                            div_result_r<=div_neg_r?(~next_r+1):next_r;
                            div_valid<=1'b1; div_active<=1'b0;
                        end else div_count<=div_count+1;
                    end
                end
            end

            logic use_latched; assign use_latched = div_active||div_valid;
            logic eff_by_zero, eff_signed_ovf; logic [31:0] eff_operand_a;
