...
module jv32_top #(
    ...
    assign slave_iram_grant = slave_wants_iram & ~(core_if_iram | core_d_iram_re | core_d_iram_we);
        ...
        if (!rst_n) begin
            ...
            case (slv_state)
                ...
                SLV_RD_RESP: begin
                    ...
                    if (slv_rd_first) slv_rdata_r <= slv_is_iram ? iram_rdata : dram_rdata;
                    if (s_axi_rready) slv_state <= SLV_IDLE;
                end
                ...
                SLV_WR_WAIT: begin
                    ...
                    if (slave_iram_grant | slave_dram_grant) slv_state <= SLV_WR_RESP;
                end

                SLV_WR_RESP: begin
                    if (s_axi_bready) slv_state <= SLV_IDLE;
                end
    ...
    assign s_axi_rdata   = slv_rd_first ? (slv_is_iram ? iram_rdata : dram_rdata) : slv_rdata_r;
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            iram_used_by_core_i_d <= core_if_iram & ~(core_d_iram_re | core_d_iram_we);
