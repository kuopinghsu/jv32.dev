#!/bin/bash
# Patch Zephyr's kconfig.py to downgrade choice-symbol warnings from fatal errors
# to non-fatal warnings.
#
# Root cause: Zephyr's kconfig.py (unchanged in 4.3 and 4.4) treats ALL Kconfig
# warnings as fatal errors, but Zephyr's own upstream SOC Kconfig files
# (Espressif esp32c6/esp32h2, etc.) generate "choice symbol" and "has direct
# dependencies" warnings during the full-tree Kconfig parse, even when building
# for an unrelated SOC like jv32.  There is no official Zephyr cmake flag to
# suppress this behavior.
#
# This script is idempotent: it is safe to run on every build.
