...
static void run_sem_test(void)
{
    ...
    printf("%-44s: min %lu avg %lu max %lu cycles\n",
           "Semaphore put with no wake", (unsigned long)min, (unsigned long)total / TEST_ITER, (unsigned long)max);
    ...
    printf("%-44s: min %lu avg %lu max %lu cycles\n",
           "Semaphore get with no contention", (unsigned long)min, (unsigned long)total / TEST_ITER, (unsigned long)max);
    ...
    printf("%-44s: min %lu avg %lu max %lu cycles\n",
           "Semaphore put with thread wake", (unsigned long)min, (unsigned long)total / TEST_ITER, (unsigned long)max);
...
{
    uint32_t i, start, delta, set_total, set_min, set_max, clr_total, clr_min, clr_max;
    ULONG    actual;
    ...
    for (i = 0; i < TEST_ITER; i++) {
        ...
        if (delta < clr_min) clr_min = delta;
    }
    printf("%-44s: min %lu avg %lu max %lu cycles\n",
    ...
    printf("%-44s: min %lu avg %lu max %lu cycles\n",
           ...
           (unsigned long)clr_min, (unsigned long)clr_total / TEST_ITER, (unsigned long)clr_max);
...
static void run_queue_test(void)
{
    ...
    printf("%-44s: min %lu avg %lu max %lu cycles\n",
           ...
           (unsigned long)put_min, (unsigned long)put_total / QUEUE_DEPTH, (unsigned long)put_max);
    ...
    printf("%-44s: min %lu avg %lu max %lu cycles\n",
           ...
           (unsigned long)get_min, (unsigned long)get_total / QUEUE_DEPTH, (unsigned long)get_max);
