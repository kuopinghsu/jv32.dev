...
# not at a functional register endpoint. Use an explicit (and less pessimistic)
# gating window for core_clk to avoid over-constraining these checks.
set_clock_gating_check -setup 0.20 [get_clocks core_clk]
...
set rst_sync_q [get_pins -quiet -of_objects [get_nets -quiet rst_sync_ff2]]
set async_reset_pins [get_pins -quiet */RN]
set async_set_pins [get_pins -quiet */SN]
if {[llength $rst_sync_q] > 0 && [llength $async_reset_pins] > 0} {
    set_false_path -from $rst_sync_q -to $async_reset_pins
}
if {[llength $rst_sync_q] > 0 && [llength $async_set_pins] > 0} {
    set_false_path -from $rst_sync_q -to $async_set_pins
}
