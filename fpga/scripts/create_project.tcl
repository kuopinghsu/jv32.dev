...
#   SCOREBOARD_EN    core scoreboard enable  (default: 1)
...
set scoreboard_en [getenv SCOREBOARD_EN "1"]
...
set ila_debug  [getenv ILA_DEBUG  "0"]
...
set_property generic "USE_CJTAG=1'b${use_cjtag} SCOREBOARD_EN=1'b${scoreboard_en}" [current_fileset]
...
if {$run_synth == "1"} {
    ...
    # Optional ILA insertion (ILA_DEBUG=1): creates ILA from explicit debug net list
    if {$ila_debug == "1"} {
        source ${script_dir}/setup_ila.tcl
    }
}
...
if {$run_impl == "1"} {
    if {$run_synth != "1"} {
        ...
        # Open the synthesized design so ILA can be inserted (if requested)
        open_run synth_1 -name synth_1
        if {$ila_debug == "1"} {
            source ${script_dir}/setup_ila.tcl
        }
    ...
    if {[file exists $bit_file]} {
        ...
        if {$ila_debug == "1"} {
            set ltx_impl_path "${proj_path}/${proj_name}.runs/impl_1/${top_module}.ltx"
            if {[catch {write_debug_probes -force $ltx_impl_path} ltx_impl_err]} {
                puts ">>> WARNING: write_debug_probes after impl failed: $ltx_impl_err"
            } else {
                puts ">>> Debug probes generated: ${ltx_impl_path}"
            }

            set ltx_dest "${proj_path}/${out_base}.ltx"
            set ltx_compat "${proj_path}/debug_probes.ltx"
            set ltx_src_candidates [list \
                "${proj_path}/${proj_name}.runs/impl_1/${top_module}.ltx" \
                "${proj_path}/${proj_name}.runs/impl_1/debug_nets.ltx" \
                "${proj_path}/debug_probes.ltx"]
            set ltx_src ""
            foreach candidate $ltx_src_candidates {
                if {[file exists $candidate]} {
                    set ltx_src $candidate
                    break
                }
            }
            if {$ltx_src ne ""} {
                file copy -force $ltx_src $ltx_dest
                file copy -force $ltx_src $ltx_compat
                puts ">>> Debug probes: ${ltx_dest}"
                puts ">>> Debug probes alias: ${ltx_compat}"
            } else {
                puts ">>> WARNING: no .ltx debug probes file was generated by Vivado."
            }
        }
