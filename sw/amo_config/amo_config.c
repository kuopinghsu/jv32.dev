...
int main(void)
{
    ...
    for (unsigned i = 0; i < 4; ++i) {
        ...
        default: PROBE(0x00c5a6af); break;
        }
        int ok = enabled ? (traps == 0) :
            (traps == 1 && cause == 2 && epc == pc &&
             tval == instructions[i] && cell == 7 && result == 85);
        if (enabled && i == 0) ok &= result == 7 && cell == 7;
        if (enabled && i == 1) ok &= result <= 1 && cell == (result ? 7u : 9u);
        if (enabled && i == 2) ok &= result == 7 && cell == 9;
        if (enabled && i == 3) ok &= result == 7 && cell == 16;
