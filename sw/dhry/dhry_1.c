...
/* Number of runs - enough for a reliable measurement (>= 2M/s equivalent cycles) */
...
int main(void) {
    ...
    if (usec_per_run > 0) {
        ...
        puts("DMIPS/MHz:    ");
        print_uint(dmips / MHZ);  /* integer part */
        puts(".");
        print_uint((dmips * 100 / MHZ) % 100);  /* two-decimal fractional part */
        puts("\n");
    }
